				...
				<form action="./sort" method="POST">
					...
					<div class="box-body">
						<div class="form-group search_box">
							<label class="control-label text-right" style="float: left;">施設選択：</label>
...
$content_js["init"][] = <<<"EOT"
	...
	table = $('#table_id').DataTable({
		...
		aoColumns: [
			{sTitle: "No", mData: "id", width:"50px", visible: false},
			{sTitle: "表示順", mData: "sort_no", width:"60px", "sClass": "right paddingRight"
				, "render": function (data, type, row) {
					return data + '<input type="hidden" name="id[]" value="' + row["id"] + '"><input type="hidden" name="category[]" value="' + row["select_category"] + '">';
				}
			},
			{sTitle: "エリア名", mData: "category_name"},
			{sTitle: "名称", mData: "title", "render": function (data, type, row) {
					return row["title"] + row["title2"];
				}
			},
			{sTitle: "名称(ローマ字)", mData: "code"
				, "render": function (data, type, row) {
					if("" != row["code2"]){
						return "/" + row["code"] + "/" + row["code2"] + "/";
					}else{
						return "/" + row["code"] + "/";
					}
				}
