...
$choices = !empty($values['choices']) ? preg_split('/\R/', $values['choices'], -1, \PREG_SPLIT_NO_EMPTY) : [];
...
<div class="box-body pad contents">
	...
	<?php endif; ?>
        ...
        <div class="form-group">
            <label class="control-label">選択肢（１行１項目）</label>
            <textarea class="form-control" name="<?php echo $template->getName(); ?>[choices]" id="<?php echo $template->getID(); ?>_choices" rows="6"><?php echo $view->escape(implode("\n", $choices)); ?></textarea>
        </div>

        <div class="form-group">
            <label class="control-label">入力時の注意点</label>
            <textarea class="form-control" name="<?php echo $template->getName(); ?>[help_text]" id="<?php echo $template->getID(); ?>_help_text" rows="3"><?php echo $view->escape($helpText); ?></textarea>
        </div>
